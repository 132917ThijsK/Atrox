...
public class BoossecondphaseProcedure {
	...
	private static void execute(@Nullable Event event, Entity entity) {
			...
			if (!entity.getPersistentData().getBoolean("CanDie")) {
				{
					...
					if (!_ent.level().isClientSide() && _ent.getServer() != null) {
						_ent.getServer().getCommands().performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, _ent.position(), _ent.getRotationVector(), _ent.level() instanceof ServerLevel ? (ServerLevel) _ent.level() : null, 4,
								_ent.getName().getString(), _ent.getDisplayName(), _ent.level().getServer(), _ent), "attribute @s minecraft:generic.max_health base set 1024");
					}
				}
				{
					Entity _ent = entity;
		...
		AtroxMod.LOGGER.info("BOOSSECONDPHASE");
	}
}
