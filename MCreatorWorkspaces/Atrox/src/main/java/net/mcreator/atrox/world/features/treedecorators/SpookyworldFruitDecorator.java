...
package net.mcreator.atrox.world.features.treedecorators;
...
public class SpookyworldFruitDecorator extends CocoaDecorator {
	public static Codec<SpookyworldFruitDecorator> CODEC = Codec.unit(SpookyworldFruitDecorator::new);
	public static TreeDecoratorType<?> DECORATOR_TYPE = new TreeDecoratorType<>(CODEC);

	@SubscribeEvent
	public static void registerTreeDecorator(RegisterEvent event) {
		event.register(ForgeRegistries.Keys.TREE_DECORATOR_TYPES, registerHelper -> registerHelper.register("spookyworld_tree_fruit_decorator", DECORATOR_TYPE));
	}

	public SpookyworldFruitDecorator() {
		super(0.2f);
	}
				...
				for (Direction direction : Direction.Plane.HORIZONTAL) {
					if (randomsource.nextFloat() <= 0.25F) {
						...
						if (context.isAir(blockpos)) {
							context.setBlock(blockpos, oriented(AtroxModBlocks.SPOOKYRACK.get().defaultBlockState(), direction1));
						}
					}
				}
			});
